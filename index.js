...
// stop the game if over
function stopGame() {
  gamerunning = false;
  const confirmRestart = window.confirm("Game Over! Restart?");
  if (confirmRestart) {
    clearBoard();
    gamerunning = true;
  } else {
...
// Clear the board array and reset the board display
function clearBoard() {
  for (let row = 0; row < BOARD_HEIGHT; row++) {
    for (let col = 0; col < BOARD_WIDTH; col++) {
      board[row][col] = 0;
      // Clear the HTML representation of the board blocks (assuming you have a game_board element)
